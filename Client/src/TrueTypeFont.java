...
public class TrueTypeFont {
    ...
    public void drawString(float x, float y,
        ...
        while (i >= startIndex && i <= endIndex) {
            ...
            if(charCurrent == '\033' && whatchars.charAt(i+1) == '['){
                int end = whatchars.indexOf("m", i+1)+1;
                String[] s = whatchars.substring(i+2, end-1).split(";");
                //System.out.println(whatchars.substring(i+2, end-1));
                for(int j = 0; j < s.length; j++){
                    try{
                        int argi = Integer.parseInt(s[j]);
                        if(argi >= 30 && argi <= 37){
                            byte r = (byte)(colors[argi-30] >> 16);
                            byte g = (byte)(colors[argi-30] >>  8);
                            byte b = (byte)(colors[argi-30] >>  0);
                            GL11.glColor3ub(r, g, b);
                        }
                    }catch (NumberFormatException e){
                        System.err.println("Invalid escape code!");
                    }
                }
